...
function doPost(e) {
  try {
    ...
    // Añadir fila con los datos (formateando teléfono como texto)
    const newRow = sheet.getLastRow() + 1;

    // Limpiar teléfono: quitar espacios y evitar que se interprete como fórmula
    let telefonoLimpio = telefono ? String(telefono).trim() : '';
    if (telefonoLimpio.startsWith('+')) {
      telefonoLimpio = "'" + telefonoLimpio; // Agregar apóstrofe al inicio para evitar fórmula
    }

    sheet.getRange(newRow, 1, 1, 9).setValues([[time, nombre, email, telefonoLimpio, motivo, ayuda, pin, rec, src]]);

    // Forzar formato de texto en la columna D (teléfono)
...
function doGet() {
  return ContentService
    .createTextOutput('Webhook OK. Usa POST para enviar datos.')
